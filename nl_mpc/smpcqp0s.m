function [nc,TAB,a,rhscon,iumin,iumax,iymin,iymax,...
                          ymin_wt,ymax_wt,dumax,SuTQ]=...
         smpcqp0s(imod,ywt,uwt,blocks,p,ulim,ylim,ylimwt)
%[nc,TAB,a,rhscon,iumin,iumax,iymin,iymax,...
%                          ymin_wt,ymax_wt,dumax,SuTQ]=...
%         smpcqp0s(imod,ywt,uwt,blocks,p,ulim,ylim,ylimwt)
%
%Designs an MPC-type controller for constrained problems.
%Assumes SOFT output constraints.
%Uses a state-space model of the process (MOD format).
%
%Output is intended as input for the MPCQPS function.
%
...
%ylim:     [ Ylow Yhigh].
%ylimwt:   [ Ylow_wt Yhigh_wt];

% Copyright by N. L. Ricker, 8/93.

% +++ Check input arguments for errors and set default values. +++

if nargin == 0
   disp('USAGE: [nc,TAB,a,rhscon,iumin,iumax,iymin,iymax,...')
   disp('                    ymin_wt,ymax_wt,dumax,SuTQ]=...')
   disp('            smpcqp0s(imod,ywt,uwt,blocks,p,ulim,ylim,ylimwt)')
   return
elseif nargin ~= 8
   disp('USAGE: [nc,TAB,a,rhscon,iumin,iumax,iymin,iymax,...')
   disp('                    ymin_wt,ymax_wt,dumax,SuTQ]=...')
   disp('            smpcqp0s(imod,ywt,uwt,blocks,p,ulim,ylim,ylimwt)')
   error('Requires 8 input arguments')
end
...
if isempty(ylimwt)
   ylimwt=[ones(1,nyi) ones(1,nyi)];
else
   [nylimwt,ncol]=size(ylimwt);
   if ncol ~= 2*nyi | nylimwt <= 0
      error('YLIMWT matrix is wrong size')
   end
end
...
umin=ulim(:,iumin)';
umin=umin(:);         % Stretches the matrix out into one long column
umax=ulim(:,iumax)';
umax=umax(:);
dumax=ulim(:,idumax)';
dumax=dumax(:);
...
ymax=ylim(:,iymax)';
ymax=ymax(:);
ymin_wt=ylimwt(:,iymin)';
ymin_wt=ymin_wt(:);
ymax_wt=ylimwt(:,iymax)';
ymax_wt=ymax_wt(:);
clear ulim ylim ylimwt      % Releases memory no longer needed.
...
leny=length(ymin_wt);
if leny > pny         % Has user specified more bounds than necessary?
   disp('WARNING:  too many rows in YLIMWT matrix.')
   disp('          Extra rows deleted.')
   ymin_wt=ymin_wt(1:pny);
   ymax_wt=ymax_wt(1:pny);
elseif leny < pny     % If fewer rows than needed, must copy last one.
   r2=[leny-nyi+1:leny];
   for i=1:round((pny-leny)/nyi)
      ymin_wt=[ymin_wt;ymin_wt(r2,:)];
      ymax_wt=[ymax_wt;ymax_wt(r2,:)];
   end
end
...
ymin_wt=diag(ymin_wt(iymin));   % Keeps only the needed weights
ymax_wt=diag(ymax_wt(iymax));
...
nymax=length(iymax);
nvars=mnu;
if nymin | nymax   % Adjusts constraint equations if there are soft constraints
   nvars=mnu+1;
   [nrow,ncol]=size(A);
   A=[A zeros(nrow,1)];
end

if nymin > 0    % Add equations for lower bound on y
   ymin=ymin(iymin);
   A=[A;-ymin_wt*Su(iymin,:) -ones(nymin,1)];
   rhscon=[rhscon;-ymin_wt*(Su(iymin,:)*dumax+ymin)];
else
   ymin=[];
end

if nymax > 0    % Add equations for upper bound on y
   ymax=ymax(iymax);
   A=[A;ymax_wt*Su(iymax,:) -ones(nymax,1)];
   rhscon=[rhscon;ymax_wt*(Su(iymax,:)*dumax+ymax)];
else
   ymax=[];
end
...
a=B'*dumax;   % This is a constant term that adds to the initial basis
              % in each QP.
if nvars > mnu
   B(nvars,nvars)=1;   % Adds new row and column, all zeros except this element
end
B=B\eye(nvars);
TAB=[-B   B*A' ;A*B   -A*B*A'];
