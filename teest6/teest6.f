...
c	 1	Feed stream 1 (pure A)
c	 2	Feed stream 2 (pure D)
c	 3	Feed stream 3 (pure E)
c	 4	Feed stream 4 (A & C)
c	 5	Compressor valve [Percent]
c	 6	Purge (stream 9)
c	 7	Separator underflow (stream 10)
c	 8	Stripper underflow (stream 11)
c	 9	Steam flow [kg/h]
...
c	19	NOT USED in this version, i.e., ignored.
...
c	This is a MEX interface routine for Matlab version 4.x

c	It is designed to emulate a Simulink S-function.
...
c		vector for SIMULINK system functions when there are no
...
c		Check the number of input arguments
	...
	if (nrhs .gt. 0 .and. nrhs .ne. 4) then
		call mexErrMsgTxt('TEest5 needs 4 input variables.')
...
c			inputs to local storage.
		...
		if (m.gt.0 .and. n.gt.0) then		
		   if (min(m,n).ne.1 .or. max(m,n) .ne. nx) then	
			call mexErrMsgTxt('X must be a vector, length '//lenx)
		   end if
		   x=mxGetPr(prhs(2))
...
c				Return dxdt.  If there are 2 LHS arguments
c				return y as the second one.  (non-standard!!)

			plhs(1)=mxCreateFull(nx,1,0)
			call mxCopyReal8ToPtr(dxdt,mxGetPr(plhs(1)),nx)  	! returns dxdt to Matlab
			if (nlhs.ge.2) then
				plhs(2)=mxCreateFull(ny,1,0)
				call mxCopyReal8ToPtr(ry,mxGetPr(plhs(2)),ny)  	! returns y to Matlab
			end if
		else
...
c	This subroutine can be used independently, i.e., without
c	the MEX interface.

c	NOTE:  all non-integer variables are double precision.

c	Inputs to routine:

c	nx	number of states.  Set to integer 31.
c	nu	number of inputs.  Set to integer 35.
c	ny	number of outputs.  Set to integer 40.
c	x	current state vector, length nx.
c	u	current input vector, length nu.

c	Outputs from routine

c	y	output vector, length ny.
c	dxdt	time derivatives of states, length nx.
