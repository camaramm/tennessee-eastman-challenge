// Level-2 c-mex S function interface for the TE problem with two temperature
// controllers.  N. L. Ricker, 12/98.  ricker@u.washington.edu

// Number continuous states = 52
// Number of inputs = 12
...
#define S_FUNCTION_NAME  temex_tc
...
#undef MDL_SET_WORK_WIDTHS   /* Change to #undef to remove function */
   ...
   *      ssNumContStates, ssSetNumDiscStates, ssSetNumRWork,
   ...
   *      mdlSetWorkWidths, even if the exact value is know before
   *      mdlIntializeSizes is called. The size to be used by the
   *      S-function should then be specified in mdlSetWorkWidths.
   */
...
static void mdlInitializeConditions(SimStruct *S)
  {
	  ...
	  if (mxIsEmpty(ssGetSFcnParam(S,0))) {
		...
		x0[49] = (float)50.;
		/* PI controller integral error states */
		x0[50] = (float)0.;
		x0[51] = (float)0.;		  
	  } else {
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
	...
	tefunc(&c__52, &rt, rx, dx);

	// Define the cost per unit production 
	prate = rx[45] * (float)211.3 / (float)46.534 + 1e-6; /* product rate (kmol/h) */
	pv_.xmeas[41] = (pv_.xmeas[19] * (float).0536 + pv_.xmeas[18] *
		    (float).0318 + pv_.xmeas[9] * (float).44791 * (
		    pv_.xmeas[28] * (float)2.209 + pv_.xmeas[30] * (float)
		    6.177 + pv_.xmeas[31] * (float)22.06 + pv_.xmeas[32] * (
		    float)14.56 + pv_.xmeas[33] * (float)17.89 + pv_.xmeas[
		    34] * (float)30.44 + pv_.xmeas[35] * (float)22.94) + 
		    prate * (pv_.xmeas[36] * (float).2206 + pv_.xmeas[37] * 
		    (float).1456 + pv_.xmeas[38] * (float).1789)) * 100 / 
		    prate;  /* cost [cents/kmol */
...
/* 	.MEX implementation of the Tennessee Eastman process model. */
/* 	Copyright by N. L. Ricker, Professor */
/* 	Department of Chemical Engineering, BF-10 */
/* 	University of Washington */
/* 	Seattle, WA 98195 */
/* 	E-mail:  ricker@cheme.washington.edu */
/* 	This version replaces inputs 10 and 11 with TC setpoints for */
/* 	the reactor and separator, respectively. */
/* 	There are a total of 51 outputs.  The "extra" ones are: */
/* 	42	Operating cost [cents/kmol product] */
/* 	43	Rate of reaction 1 [kmol G produced/h] */
/* 	44	Rate of reaction 2 [kmol H produced/h] */
/* 	45	Rate of generation of F in reactions 3 and 4 [kmol F produced/h] */
/* 	46	Partial pressure of A in reactor [kPa] */
/* 	47	Partial pressure of C in reactor [kPa] */
/* 	48	Partial pressure of D in reactor [kPa] */
/* 	49	Partial pressure of E in reactor [kPa] */
/* 	50	True (delay and noise-free) mole % G in product (stream 11) */
/* 	51	True (delay and noise-free) mole % H in product (stream 11) */
/* --------------------------------------------------------------------- */
...
/* 	Next 7 lines inserted by NL Ricker to define reaction rate */
/* 	and partial pressure outputs */
    pv_.xmeas[42] = teproc_.rr[0] * (float).454;
    pv_.xmeas[43] = teproc_.rr[1] * (float).454;
    pv_.xmeas[44] = teproc_.rr[2] * (float).454;
    pv_.xmeas[45] = teproc_.ppr[0] / (float)760. * (float)101.325;
    pv_.xmeas[46] = teproc_.ppr[2] / (float)760. * (float)101.325;
    pv_.xmeas[47] = teproc_.ppr[3] / (float)760. * (float)101.325;
    pv_.xmeas[48] = teproc_.ppr[4] / (float)760. * (float)101.325;
...
/* 	Next 2 lines inserted by N. L. Ricker to define delay */
/* 	and noise-free product composition outputs */
    pv_.xmeas[49] = xcmp[39];
    pv_.xmeas[50] = xcmp[40];
    if (*time == 0.) {
...
/* 		All (noisy) measurements have been computed.  Now */
/* 		do the PI loops. */
    yp[51] = pv_.xmv[9] - pv_.xmeas[8];
/* error in reactor temperature [C] */
    yp[52] = pv_.xmv[10] - pv_.xmeas[10];
/* error in separator temperature [C] */
    pv_.xmv[9] = (float)41.10581288 - (yp[51] + yy[51] * (float)8.) * (float)
	    8.;
/* PI law for reactor temp control */
    pv_.xmv[10] = (float)18.11349055 - (yp[52] + yy[52] * (float)4.) * (
	    float)4.;
/* 		Prevent integral windup. */
/* PI law for separator temp control */
    if (pv_.xmv[9] <= (float)0. && yp[51] > (float)0.) {
	pv_.xmv[9] = (float)0.;
	yp[51] = (float)0.;
    } else if (pv_.xmv[9] >= (float)100. && yp[51] < (float)0.) {
	pv_.xmv[9] = (float)100.;
	yp[51] = (float)0.;
    }
    if (pv_.xmv[10] <= (float)0. && yp[52] > (float)0.) {
	pv_.xmv[10] = (float)0.;
	yp[52] = (float)0.;
    } else if (pv_.xmv[10] >= (float)100. && yp[52] < (float)0.) {
	pv_.xmv[10] = (float)100.;
	yp[52] = (float)0.;
    }
    for (i__ = 1; i__ <= 12; ++i__) {
